...
object Database:
  ...
  lazy val transactor: Resource[IO, HikariTransactor[IO]] =
    for {
      ...
      transactor <- transactorFor(dbUrl, dbUsername, dbPassword)
    } yield transactor
  ...
  def transactorFor(
    ...
    for {
      _ <- Resource.eval(migrate(url, username, password))
      ...
      transactor <- HikariTransactor
        .fromHikariConfig[IO](hikariConfig, logHandler.some)
    } yield transactor
  ...
  val logHandler: LogHandler[IO] = new LogHandler[IO] {
    def run(logEvent: LogEvent): IO[Unit] = logEvent match {
      case s: Success =>
        log.info(
          s"[SQL] Success; Exec: ${s.exec}, Processing: ${s.processing}, Params: ${s.params}:\n${s.sql}"
        )
      case e: ExecFailure =>
        log.error(e.failure)(
          s"[SQL] Exec Failure: ${e.failure.getMessage}; Exec: ${e.exec}, Params: ${e.params}:\n${e.sql}"
        )
      case e: ProcessingFailure =>
        log.error(e.failure)(
          s"[SQL] Processing Failure: ${e.failure.getMessage}; Exec: ${e.exec}, Processing: ${e.processing}, Params: ${e.params}:\n${e.sql}"
        )
    }
  }

  def migrate(url: String, username: String, password: String): IO[Unit] = IO {
    val flyway = Flyway
      .configure()
